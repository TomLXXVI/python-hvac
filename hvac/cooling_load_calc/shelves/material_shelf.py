...
class MaterialShelf:
    ...
    def add(cls, *records: MaterialTuple) -> None:
        ...
        Notes
        -----
        It may happen that k is not available, but instead the unit thermal
        resistance R of the material is specified. In that case, set k (index 1)
        ...
        the standard SI-units for thermal conductivity ('W / (m * K)'), mass
        density ('kg / m ** 3'), specific heat ('J / (kg * K)'), and unit
    ...
    def replace(cls, ID: str, record: MaterialTuple) -> None:
        """Replaces the material with the given ID with another material, which
