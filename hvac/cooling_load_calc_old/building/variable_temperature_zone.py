...
from hvac.cooling_load_calc_old.building.fixed_temperature_zone import (
    LocalVentilation
)
from hvac.cooling_load_calc_old.building.ventilation_zone import (
    VentilationZone
)
...
from hvac.cooling_load_calc_old.core.thermal_models import (
    ThermalZoneModel
)
...
class VariableTemperatureZone:
    ...
    def _init_values(self) -> list[list[Quantity]]:
        ...
        T_sa_avg = []
        for ebe in self.ext_build_elems.values():
            T_sa_avg.append(ebe.ext_surf.T_sa(0))
            num_nodes = len(ebe.model)
            T_init_sub_lst = [ebe.ext_surf.T_sa(0)] * num_nodes
        ...
        for t_sol_sec, T_zone in zip(t_sol_axis, arr_T_zone):
            ...
            for door in edr_lst:
                Q_dot_edr = (
                    ...
                    * (door.ext_surf.T_db(t_sol_sec).to('K') - T_zone.to('K'))
                )
