...
if TYPE_CHECKING:
    ...
    from hvac.cooling_load_calc_old.core.fenestration import Window
    from hvac.cooling_load_calc_old.core.building_element import (
        ExteriorBuildingElement,
        InteriorBuildingElement
    )
    from hvac.cooling_load_calc_old.building.fixed_temperature_zone import LocalVentilation
...
class ThermalStorageNode(TemperatureNode):
    ...
    def _b(self, T: list[float], t_sol_sec: float) -> float:
        ...
        if self.ext_doors:
            Q_dot_edr = sum(
                (edr.F_rad * edr.UA * edr.ext_surf.T_sa(t_sol_sec).to('K')).to('W').m
                for edr in self.ext_doors
            )
        if self.int_build_elems:
            Q_dot_ibe = sum(
...
class ZoneAirNode(TemperatureNode):
    ...
    def _b(self, T: list[float], t_sol_sec: float) -> float:
        ...
        if self.ext_doors:
            Q_dot_edr = sum(
                ((1 - edr.F_rad) * edr.UA * edr.ext_surf.T_sa(t_sol_sec).to('K')).to('W').m
                for edr in self.ext_doors
            )
        if self.int_build_elems:
            Q_dot_ibe = sum(
...
class _ThermalZoneModelBuilder:
    ...
    def _create_ebe_nodes(self) -> None:
        ...
        for ebe in self.ebe_lst:
            ...
            ltn = self._transform(ltn_red, ebe.net_area, ebe.ID, ebe.ext_surf.T_sa)
