...
`hvac.cooling_load_calc.core.building_element` and class `FixedTemperatureZone`
in `hvac.cooling_load_calc.building.fixed_temperature_zone`).

References
----------
[1] Kiusalaas, J. (2013). NUMERICAL METHODS IN ENGINEERING WITH PYTHON3.
...
from hvac.cooling_load_calc_old.core.construction_assembly import (
...
class ExteriorBuildingElementLTN(list[ExteriorBuildingElementNode]):
    """Linear thermal network model of an exterior building element. 
    
    An `ExteriorBuildingElementLTN` object is a list of `ExteriorBuildingElement
    Node` objects. The first element is always an `ExteriorSurfaceNode` object
    and the last element is always an `InteriorSurfaceNode` object. Between the 
    first and the last element, the list contains at least one, or more 
    `BuildingMassNode` objects.
    """
