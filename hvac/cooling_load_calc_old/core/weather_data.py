...
class WeatherData:
    ...
    Depending on the constructor method and its parameters, there a three 
    possible ways to create a `WeatherData` object.
    ...
    def T_db(self, t_sol_sec: float) -> Quantity:
        """
        Returns the outdoor air dry-bulb temperature at the specified solar time
        of the design day.
    ...
    def T_wb(self, t_sol_sec: float) -> Quantity:
        """
        Returns the outdoor air wet-bulb temperature at the specified solar time
        of the design day.
    ...
    def I_T(
        ...
        """
        Returns the hourly solar irradiation components on a tilted surface at
        the given solar time hour `hr1`, which specifies the start time of a 1 
        hour-period.
...
class ExteriorSurface(Surface):
    """
    Represents the exterior surface of an opaque exterior building element and
    of windows. This class is used internally in the class 
    `ExteriorBuildingElement` in module building_element.py and in the class 
    `Window` in module fenestration.py.

    This class contains the methods needed to calculate the temperature and solar
    radiation at a given time of the design day, and which are needed to determine
