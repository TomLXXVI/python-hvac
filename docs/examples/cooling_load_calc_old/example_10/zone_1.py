"""
COOLING LOAD CALCULATION OF ZONE 1 IN THE TWO-ZONE BUILDING.

In this module the space/thermal zone "Zone 1" of the two-zone building is
created and configured.

The building envelope that surrounds the space/thermal zone and the interior
thermal mass in the space/thermal zone are created inside a separate class
`_SpaceConstruction`. This class also creates and returns the space/thermal zone.

Next, the local ventilation/air infiltration of the space/thermal zone is
configured inside a class `_SpaceVentilation`.

Lastly, the internal heat gains of the space/thermal zone are created and added
to the space/thermal zone inside a class `_InternalHeatGains`.

The class `Zone1` instantiates these classes in its `__init__()`-method. All
data needed to configure the space/thermal zone is contained inside these
classes. The only external input data needed to instantiate the `Zone1` class is
a `WeatherData` object, the indoor air design temperature in the space/thermal
zone, and the ventilation zone (`VentilationZone` object) to which the
space/thermal zone belongs. The `WeatherData` object contains the geographic
location, the date of the design day, and the climatic design information.

After instantiation of the `TwoZoneBuilding` class in module `building.py`,
calling the method `cooling_load` on the `Zone1` object returns a Pandas
DataFrame with the hourly heat gains and cooling loads of zone 1 during the
specified design day. Note that time is expressed by time indexes in solar time.
Method `plot_cooling_load` takes the Pandas DataFrame returned by method
`cooling_load` and plots the sensible cooling load, the latent cooling load, and
the total cooling load of zone 1 in a line chart.
"""
...
class _SpaceConstruction:
    ...
    def __init__(
        ...
        self.west_wall: ExteriorBuildingElement | None = None
        self.north_wall: ExteriorBuildingElement | None = None
        self.west_roof: ExteriorBuildingElement | None = None
    ...
    def _interior_floor_area(self) -> Quantity:
        ...
        w = self.space_width
        w -= self.west_wall.constr_assem.thickness
        l = self.space_length
    ...
    def _create_west_wall(self) -> ExteriorBuildingElement:
    ...
    def _create_west_roof(self) -> ExteriorBuildingElement:
        # Get the construction assembly:
    ...
    def build(self) -> FixedTemperatureZone:
        ...
        self.west_roof = self._create_west_roof()
        ...
        self.zone.add_ext_build_elem(
            ...
            self.north_wall, self.west_roof
        )
        ...
        walls = [self.south_wall, self.north_wall, self.west_wall]
