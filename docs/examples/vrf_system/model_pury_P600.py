"""
CREATE THE MODEL OF A MITSUBISHI R2 SERIES VRF-SYSTEM
WITH OUTDOOR UNIT PURY P-600 YSLM-A1(-BS)
IN HEATING MODE
"""
from hvac.vrf_system import model

# ------------------------------------------------------------------------------
# INSTANTIATE VRF MODEL CREATOR
# ------------------------------------------------------------------------------
vrf_model_creator = model.VRFModelCreator(
    Tia_list=[15.0, 20.0, 25.0, 27.0],
    # indoor air temperatures for which input power ratios are given
    Tao_limits=(-20.0, 15),
    # outdoor air temperature limits
)


# ------------------------------------------------------------------------------
# TEMPERATURE CORRECTION FOR CAPACITY RATIO
# ------------------------------------------------------------------------------
points = [
    (-20.0, 0.5 + 0.100),
    (-19.0, 0.5 + 0.113),
    (-18.0, 0.5 + 0.125),
    (-17.0, 0.5 + 0.136),
    (-16.0, 0.5 + 0.149),
    (-15.0, 0.5 + 0.163),
    (-14.0, 0.5 + 0.177),
    (-13.0, 0.5 + 0.187),
    (-12.0, 0.5 + 0.201),
    (-11.0, 0.5 + 0.213),
    (-10.0, 0.5 + 0.227),
    (-9.00, 0.5 + 0.247),
    (-8.00, 0.5 + 0.265),
    (-7.00, 0.5 + 0.286),
    (-6.00, 0.5 + 0.305),
    (-5.00, 0.5 + 0.324),
    (-4.00, 0.5 + 0.343),
    (-3.00, 0.5 + 0.360),
    (-2.00, 0.5 + 0.380),
    (-1.00, 0.5 + 0.401),
    (0.000, 0.5 + 0.417),
    (1.000, 0.5 + 0.438),
    (2.000, 0.5 + 0.458),
    (3.000, 0.5 + 0.476),
    (4.000, 0.5 + 0.500),
    (15.00, 0.5 + 0.500)
]

vrf_model_creator.create_QT_curve(points)


# ------------------------------------------------------------------------------
# TEMPERATURE CORRECTION FOR POWER INPUT RATIO
# ------------------------------------------------------------------------------

points_15 = [
    (-20.0, 0.4 + 0.500),
    (-19.0, 0.4 + 0.500),
    (-18.0, 0.4 + 0.504),
    (-17.0, 0.4 + 0.511),
    (-16.0, 0.4 + 0.514),
    (-15.0, 0.4 + 0.522),
    (-14.0, 0.4 + 0.528),
    (-13.0, 0.4 + 0.531),
    (-12.0, 0.4 + 0.541),
    (-11.0, 0.4 + 0.549),
    (-10.0, 0.4 + 0.556),
    (-9.00, 0.4 + 0.565),
    (-8.00, 0.4 + 0.573),
    (-7.00, 0.4 + 0.583),
    (-6.00, 0.4 + 0.593),
    (-5.00, 0.4 + 0.602),
    (-4.00, 0.4 + 0.614),
    (-3.00, 0.4 + 0.628),
    (-2.00, 0.4 + 0.638),
    (-1.00, 0.4 + 0.652),
    (0.000, 0.4 + 0.666),
    (1.000, 0.4 + 0.680),
    (2.000, 0.4 + 0.693),
    (3.000, 0.4 + 0.708),
    (4.000, 0.4 + 0.723),
    (5.000, 0.4 + 0.672),
    (6.000, 0.4 + 0.684),
    (7.000, 0.4 + 0.694),
    (8.000, 0.4 + 0.703),
    (9.000, 0.4 + 0.715),
    (10.00, 0.4 + 0.724),
    (11.00, 0.4 + 0.734),
    (12.00, 0.4 + 0.744),
    (13.00, 0.4 + 0.751),
    (14.00, 0.4 + 0.680),
    (15.00, 0.4 + 0.630)
]

vrf_model_creator.create_WT_curve(15, points_15)


points_20 = [
    (-20.0, 0.4 + 0.583),
    (-19.0, 0.4 + 0.587),
    (-18.0, 0.4 + 0.590),
    (-17.0, 0.4 + 0.593),
    (-16.0, 0.4 + 0.601),
    (-15.0, 0.4 + 0.603),
    (-14.0, 0.4 + 0.611),
    (-13.0, 0.4 + 0.618),
    (-12.0, 0.4 + 0.625),
    (-11.0, 0.4 + 0.631),
    (-10.0, 0.4 + 0.640),
    (-9.00, 0.4 + 0.652),
    (-8.00, 0.4 + 0.659),
    (-7.00, 0.4 + 0.670),
    (-6.00, 0.4 + 0.679),
    (-5.00, 0.4 + 0.693),
    (-4.00, 0.4 + 0.703),
    (-3.00, 0.4 + 0.718),
    (-2.00, 0.4 + 0.731),
    (-1.00, 0.4 + 0.744),
    (0.000, 0.4 + 0.758),
    (1.000, 0.4 + 0.772),
    (2.000, 0.4 + 0.785),
    (3.000, 0.4 + 0.804),
    (4.000, 0.4 + 0.821),
    (5.000, 0.4 + 0.723),
    (6.000, 0.4 + 0.602),
    (7.000, 0.4 + 0.561),
    (8.000, 0.4 + 0.521),
    (9.000, 0.4 + 0.490),
    (10.00, 0.4 + 0.460),
    (11.00, 0.4 + 0.440),
    (12.00, 0.4 + 0.424),
    (13.00, 0.4 + 0.410),
    (14.00, 0.4 + 0.400),
    (15.00, 0.4 + 0.391)
 ]

vrf_model_creator.create_WT_curve(20, points_20)


points_25 = [
    (-20.0, 0.4 + 0.672),
    (-19.0, 0.4 + 0.676),
    (-18.0, 0.4 + 0.678),
    (-17.0, 0.4 + 0.683),
    (-16.0, 0.4 + 0.686),
    (-15.0, 0.4 + 0.694),
    (-14.0, 0.4 + 0.696),
    (-13.0, 0.4 + 0.703),
    (-12.0, 0.4 + 0.713),
    (-11.0, 0.4 + 0.720),
    (-10.0, 0.4 + 0.731),
    (-9.00, 0.4 + 0.743),
    (-8.00, 0.4 + 0.752),
    (-7.00, 0.4 + 0.762),
    (-6.00, 0.4 + 0.775),
    (-5.00, 0.4 + 0.785),
    (-4.00, 0.4 + 0.729),
    (-3.00, 0.4 + 0.682),
    (-2.00, 0.4 + 0.640),
    (-1.00, 0.4 + 0.590),
    (0.000, 0.4 + 0.548),
    (1.000, 0.4 + 0.507),
    (2.000, 0.4 + 0.468),
    (3.000, 0.4 + 0.434),
    (4.000, 0.4 + 0.400),
    (5.000, 0.4 + 0.373),
    (6.000, 0.4 + 0.360),
    (7.000, 0.4 + 0.344),
    (8.000, 0.4 + 0.328),
    (9.000, 0.4 + 0.314),
    (10.00, 0.4 + 0.301),
    (11.00, 0.4 + 0.287),
    (12.00, 0.4 + 0.280),
    (13.00, 0.4 + 0.272),
    (14.00, 0.4 + 0.268),
    (15.00, 0.4 + 0.260)
]

vrf_model_creator.create_WT_curve(25, points_25)


points_27 = [
    (-20.0, 0.4 + 0.703),
    (-19.0, 0.4 + 0.706),
    (-18.0, 0.4 + 0.710),
    (-17.0, 0.4 + 0.716),
    (-16.0, 0.4 + 0.720),
    (-15.0, 0.4 + 0.728),
    (-14.0, 0.4 + 0.735),
    (-13.0, 0.4 + 0.740),
    (-12.0, 0.4 + 0.748),
    (-11.0, 0.4 + 0.755),
    (-10.0, 0.4 + 0.764),
    (-9.00, 0.4 + 0.771),
    (-8.00, 0.4 + 0.724),
    (-7.00, 0.4 + 0.684),
    (-6.00, 0.4 + 0.639),
    (-5.00, 0.4 + 0.600),
    (-4.00, 0.4 + 0.560),
    (-3.00, 0.4 + 0.525),
    (-2.00, 0.4 + 0.491),
    (-1.00, 0.4 + 0.460),
    (0.000, 0.4 + 0.428),
    (1.000, 0.4 + 0.397),
    (2.000, 0.4 + 0.367),
    (3.000, 0.4 + 0.341),
    (4.000, 0.4 + 0.315),
    (5.000, 0.4 + 0.295),
    (6.000, 0.4 + 0.280),
    (7.000, 0.4 + 0.268),
    (8.000, 0.4 + 0.256),
    (9.000, 0.4 + 0.243),
    (10.00, 0.4 + 0.231),
    (11.00, 0.4 + 0.222),
    (12.00, 0.4 + 0.210),
    (13.00, 0.4 + 0.204),
    (14.00, 0.4 + 0.194),
    (15.00, 0.4 + 0.186)
]

vrf_model_creator.create_WT_curve(27, points_27)


# ------------------------------------------------------------------------------
# CREATE CAPFT-FUNCTION
# ------------------------------------------------------------------------------
vrf_model_creator.create_CAPFT_function()

# ------------------------------------------------------------------------------
# CREATE EIRFT-FUNCTION
# ------------------------------------------------------------------------------
vrf_model_creator.create_EIRFT_function()


# ------------------------------------------------------------------------------
# CREATE EIRFPLR-FUNCTION
# ------------------------------------------------------------------------------
points = [
    (300, 0.2 + 0.218),
    (320, 0.2 + 0.239),
    (360, 0.2 + 0.288),
    (400, 0.2 + 0.350),
    (440, 0.2 + 0.419),
    (480, 0.2 + 0.502),
    (520, 0.2 + 0.589),
    (560, 0.2 + 0.689),
    (600, 0.2 + 0.800),
    (640, 0.2 + 0.786),
    (680, 0.2 + 0.775),
    (720, 0.2 + 0.764),
    (760, 0.2 + 0.752),
    (800, 0.2 + 0.740),
    (840, 0.2 + 0.727),
    (880, 0.2 + 0.716),
    (900, 0.2 + 0.712)
]

vrf_model_creator.create_EIRFPLR_function(600, points)


# ------------------------------------------------------------------------------
# CREATE CR_corr FUNCTION
# ------------------------------------------------------------------------------
points = [
    (300, 0.50),
    (600, 1.00),
    (900, 1.08)
]

vrf_model_creator.create_CR_corr_function(600, points)


# ------------------------------------------------------------------------------
# PIPING LENGTH CORRECTION
# ------------------------------------------------------------------------------
points = [
    (10, 0.8 + 0.197),
    (20, 0.8 + 0.191),
    (40, 0.8 + 0.178),
    (60, 0.8 + 0.166),
    (80, 0.8 + 0.154),
    (100, 0.8 + 0.143),
    (120, 0.8 + 0.132),
    (140, 0.8 + 0.121),
    (160, 0.8 + 0.112),
    (175, 0.8 + 0.105)
]

vrf_model_creator.create_Leq_corr_function(points)


# ------------------------------------------------------------------------------
# HEAT PUMP RUNTIME FRACTION
# ------------------------------------------------------------------------------
vrf_model_creator.create_HPRTF_function(0.5)

# ------------------------------------------------------------------------------
# DEFROST CYCLE CORRECTION
# ------------------------------------------------------------------------------
points = [
    (6, 1.00),
    (4, 0.94),
    (2, 0.84),
    (1, 0.86),
    (0, 0.87),
    (-2, 0.88),
    (-4, 0.90),
    (-6, 0.90),
    (-8, 0.93),
    (-10, 0.93),
    (-20, 0.93)
]

vrf_model_creator.create_defrost_corr_function(points)

# ------------------------------------------------------------------------------
# SAVE THE VRF-MODEL
# ------------------------------------------------------------------------------
vrf_model_creator.save_model("./models/pury-p600-heating.pickle")
