...
class ConstructionAssemblies:
    ...
    def exterior_wall_light(
        ...
        ConstructionAssembly
        """
        # Create the layers of the construction assembly:
        ...
        # (each slice will represent a temperature node; by default, a layer has
        # only one temperature node in which all heat capacity of the layer is
        # concentrated):
        insulation.slices = 7
    ...
    def sky_light() -> WindowThermalProperties:
        ...
        exposition hall, which are bundled in a `WindowThermalProperties`
        object.
        """
...
class Construction:
    """Class that adds the building construction to the single-zone building."""
    light_wall: ConstructionAssembly
    heavy_wall: ConstructionAssembly
    roof: ConstructionAssembly

    @classmethod
    def create_construction_assemblies(cls, T_int: Quantity, T_ext: Quantity):
    ...
    def add(cls, space: Space, is_heavy: bool = False) -> None:
        ...
        a choice can be made between a heavy-weight wall construction (concrete
        exterior walls) or a light-weight wall construction.
        """
        if is_heavy:
            wall = cls.heavy_wall
        else:
...
class ExpositionHall:
    ...
    def _create_single_space(
        ...
        Construction.create_construction_assemblies(
            T_int=interior_temperature_schedule.base_value,
        ...
        # Create the ventilation zone; add the space to this zone:
        vz = VentilationZone.create(ID='exposition_hall')
        vz.add_space(space)
        # Configure the ventilation characteristics of the space (use the default
        # values):
        space.configure_ventilation()
        # Create the building entity; add the ventilation zone to the building
        # entity:
        be = BuildingEntity.create(ID='exposition_hall')
        be.add_ventilation_zone(vz)
        # Create the building; add the building entity to the building:
        bu = Building.create(ID='exposition_hall')
        bu.add_building_entity(be)
        # Only return the single space:
        return space
    ...
    def _create_T_setpoint_schedule(
        ...
        set to the value of `T_economy`. During the other hours of the day, the
        setpoint is set to the value of `T_comfort`.
        """
        # Create temperature schedule with a base value of `T_comfort`:
        schedule = TemperatureSchedule.create(
            ID='sch_space_air',
            base_value=T_comfort
        )
        # Change setpoint to `T_economy` between 0 and 8 h:
        for t in range(8):
            schedule.set_value(t, T_economy)

        # Change setpoint again to `T_economy` between 18 and 24 h:
    ...
    def _create_internal_heat_gains(
        ...
        """Creates the internal heat gains in the space. Only internal heat
        gain by people is being considered. Between 18 h and 8 h the number of
        people in the space is equal to the value of `people_min`. During
        other hours of the day, the number of people is equal to the value of
        `people_max`.
        """
...
def main():
    ...
    # Show a table of the heat gains and the cooling loads on an hourly basis:
    Q_gains = hall.get_heat_gains(unit='kW')
    with pd.option_context('display.width', None):
        print(Q_gains)

    # Show a diagram of the hourly cooling load and the time course of indoor
    # air temperature:
