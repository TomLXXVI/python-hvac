...
def create_machine(logger) -> SingleStageVaporCompressionMachine:
    """Create the components of the single-stage vapor compression machine
...
def create_inputs() -> list[tuple[Quantity, Quantity]]:
    ...
    """
    cnd_m_dot_air_rng = Q_(np.arange(2000, 4500, 500), 'kg / hr')
...
def analyze_performance(inputs: tuple[Quantity, Quantity]) -> dict[str, float]:
    ...
    Parameter `inputs` is a tuple of condenser air mass flow rate and compressor
    speed.
    ...
    cnd_m_dot_air, n_cmp = inputs[0], inputs[1]
    machine.set_operating_conditions(
        ...
        cnd_air_in=HumidAir(Tdb=Q_(30.0, 'degC'), RH=Q_(50, 'pct')),
        dT_sh=Q_(10, 'K'),
        n_cmp=n_cmp
    )
    try:
        ...
        return {
            'cnd_m_dot_air [kg/h]': cnd_m_dot_air.to('kg / hr').m,
...
def main():
    ...
    log_file_path = pathlib.Path(
        ...
        "docs/examples/vapor_compression/log_files/vcm_ex07_3a.log"
    )
    ...
    df.to_excel('./analysis_results/07_multi_analysis_3.ods')
